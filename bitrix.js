/* 
 * bitrix.js - ПРУТКОН ОС | CRM Integration Master (v18.25.4)
 * Полная поддержка AI-парсинга, поиска и интеграции сделок.
 */

console.log("🛰 Bitrix24 Integration v18.25.4 loading...");
...
window.fetchBitrixContactDetails = async function(dealId) {
    ...
    if (deal?.CONTACT_ID) {
        ...
        if (c) {
            document.getElementById('card-client-name').innerText = `${c.NAME} ${c.LAST_NAME || ''}`;
            document.getElementById('card-client-phone').innerText = c.PHONE?.[0]?.VALUE || '---';
        }
    }
};
