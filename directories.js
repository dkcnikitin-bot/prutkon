        ...
        filtered.forEach(r => {
            ...
            html += `
                <tr class="${isSelected ? 'row-selected' : ''}" ondblclick="window.editDirectoryRecord('${r.id}')">
                    ...
                    ${schema.map(f => {
                        let val = r[f] || '-';
                        if (window.activeDirCategory === 'metal' && val !== '-') {
                            const isCurrency = ['price', 'sum_no_vat', 'sum_vat', 'price_m_no_vat', 'price_m_vat', 'delivery_m_no_vat', 'delivery_m_vat', 'total_price_m_no_vat', 'total_price_m_vat'].includes(f);
...
window.renderDirSchemaFields = (data) => {
    ...
    const steelTypesHtml = (window.steelTypes || []).map(s => `<option value="${s}">`).join('');
    const suppliersHtml = suppliers.map(s => `<option value="${s}">`).join('');
    ...
    if (window.activeDirCategory === 'metal') {
        ...
        cont.innerHTML = schema.map(f => `
            <div class="form-group">
                ...
                <input type="text" name="${f}" class="form-control dir-schema-input" value="${data[f] || ''}">
            </div>
        `).join('');
    }
};
